...
{
    public class DateTimeSharpD : IComparable
    {
        ...
        public int CompareTo(object Item)
        {
            ...
            DateTimeSharpD tempInput = Item as DateTimeSharpD;
        ...
        public static implicit operator DateTime(DateTimeSharpD Parameter)
        {
            return Parameter.Value;
        }

        public static implicit operator DateTimeSharpD(DateTime Value)
        {
            return new DateTimeSharpD(Value);
        }
        ...
        public DateTimeSharpD(DateTime Value)
        {
            this.Value = Value;
            InputObject = Value;
        }

        public DateTimeSharpD(string Value)
        {
            this.Value = ParseDateTime(Value);
            InputObject = Value;
        }

        public DateTimeSharpD(object InputObject)
        {
            if (InputObject == null)
                ...
                if (PropertyMapping.ContainsKey(name))
                {
                    foreach (string property in PropertyMapping[name])
                    {
                        if (input.Properties[property] != null && input.Properties[property].Value != null)
                        {
                            ...
                            try { return new DateTimeSharpD(input.Properties[property].Value).Value; }
                            catch { }
                        }
                    }
                }
