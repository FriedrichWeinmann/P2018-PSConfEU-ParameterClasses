...
# 6. (In)comparable
code "$presentationRoot\C#\DateTimeSharpD.cs"
function Get-SharpTest {
    [CmdletBinding()]
    param (
        [DateTimeSharpD]$Timestamp
    )
    $Timestamp
}
$object = Get-SharpTest -Timestamp "-2d 3h 15m 52s"
...
# -> Growth and documentation issue
code "$presentationRoot\C#\DateTimeSharpE.cs"
code "$presentationRoot\C#\ParameterClassAttribute.cs"
code "$presentationRoot\C#\ParameterContractAttribute.cs"

# Searching for parameter classes
$attributeType = Find-PSMDType -FullName ParameterClasses.ParameterClassAttribute
$classes = Find-PSMDType | Where-Object { $_.CustomAttributes.AttributeType -contains $attributeType }
$classes
$classes | Get-ParameterClassInfo
 ...
 [DbaInstance]"Foo"
 [DbaInstance]"(localdb)\bar"
 [DbaInstance]"Server = Foo;"
 [DbaInstance](Connect-DbaInstance -SqlInstance .)

 # On Scale & Maintenance
 # Testing
 ...
 [PSFComputer]"Foo"
 [PSFComputer]"::1"
 [PSFComputer]([System.Net.Dns]::Resolve("localhost"))
 [PSFDateTime]"-2d"

 # Interdependence & Interop